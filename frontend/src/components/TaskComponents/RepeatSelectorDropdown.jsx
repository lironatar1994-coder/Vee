...
import { hebrewDayNames, hebrewMonthNames, repeatOptions as baseRepeatOptions } from './utils.jsx';

const RepeatSelectorDropdown = ({ isOpen, onClose, anchorRef, repeatRule, onSelect }) => {
    const dropdownRef = useRef(null);
    const [pos, setPos] = useState({ top: 0, left: 0, visible: false });

    const today = new Date();
    const repeatOptions = [
        ...baseRepeatOptions.filter(o => o.value !== 'none'),
    ...
    ].filter((v, i, a) => a.findIndex(t => (t.value === v.value)) === i); // dedupe and keep custom labels
        ...
        >
            {repeatOptions.map((opt, i) => (
                <button
                    ...
                    onClick={(e) => {
                        e.stopPropagation();
                        onSelect(opt.value === 'custom' ? null : opt.value);
                        onClose();
                    }}
                    style={{
                        ...
                        borderBottom: i !== repeatOptions.length - 1 ? '1px solid var(--border-color)' : 'none',
                        transition: 'background 0.15s',
                        borderRadius: '6px'
                    }}
                    onMouseEnter={e => {
                        if (repeatRule !== opt.value) e.currentTarget.style.background = 'var(--dropdown-hover)';
                    }}
                    onMouseLeave={e => {
                        if (repeatRule !== opt.value) e.currentTarget.style.background = 'transparent';
                        else e.currentTarget.style.background = 'var(--dropdown-selected)';
                    }}
                >
                    <RefreshCw size={14} style={{ color: repeatRule === opt.value ? 'var(--primary-color)' : 'var(--text-secondary)' }} />
                    <span style={{ fontSize: '0.87rem', color: 'var(--text-primary)', fontWeight: repeatRule === opt.value ? 600 : 400 }}>{opt.label}</span>
                </button>
            ))}
        </div>,
        document.body
...
export default RepeatSelectorDropdown;
